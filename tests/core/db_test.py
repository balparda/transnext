...
class _MockAPI:
  ...
  def GetEmbeddings(self) -> dict[str, str]:  # noqa: PLR6301
    """Return mock embeddings."""  # noqa: DOC201
    return {}
...
def testVariationSeedAutoGenerated() -> None:
  ...
  result: db.AIMetaType = db.AIMetaTypeFactory(
    {'v_seed': db.AIMetaVariationSeedType(seed=0, percent=50)}
  )
  assert result['v_seed'] is not None
  assert result['v_seed']['seed'] > 0
  assert result['v_seed']['percent'] == 50
...
def testVariationSeedInvalidRaises() -> None:
  ...
  with pytest.raises(db.Error, match=r'Invalid.*v_seed.*value'):
    db.AIMetaTypeFactory(
      {'v_seed': db.AIMetaVariationSeedType(seed=base.SD_MAX_SEED + 1, percent=50)}
    )
...
def testRefreshDBModels(tmp_path: pathlib.Path) -> None:
  ...
  ai_db = db.AIDatabase(_MakeAppConfig(tmp_path))
  model_path = tmp_path / 'm.st'
  model_path.write_bytes(b'')
  model: db.AIModelType = _MakeModel(h='hash1', name='model-1', path=str(model_path))
...
def testRefreshDBModelsSkipDuplicatePath(tmp_path: pathlib.Path) -> None:
  ...
  model_path.write_bytes(b'')
  model: db.AIModelType = _MakeModel(h='hash1', name='model-1', path=str(model_path))
  ai_db._db['models']['hash1'] = _MakeModel(h='hash1', name='existing', path=str(model_path))
  # same path and same hash
  api = _MockAPI(models=[model])
  ai_db.RefreshDBModels(api)
  # only existing model stays, new one has same path so skipped
  assert 'hash1' in ai_db._db['models']
  assert ai_db._db['models']['hash1']['name'] == 'existing'
...
def testRefreshDBModelsEmptyHash(tmp_path: pathlib.Path) -> None:
  ...
  model: db.AIModelType = _MakeModel(h='', name='model-1', path=str(model_path))
  api = _MockAPI(models=[model])
...
def testGetModelHashFetchesFromAPI(tmp_path: pathlib.Path) -> None:
  ...
  ai_db = db.AIDatabase(_MakeAppConfig(tmp_path))
  model_path = tmp_path / 'new.st'
  model_path.write_bytes(b'')
  model: db.AIModelType = _MakeModel(h='hash1', name='XLB_v10', path=str(model_path))
  api = _MockAPI(models=[model])
...
def testSyncWithAPIRefreshesModels(tmp_path: pathlib.Path) -> None:
  ...
  model_path = tmp_path / 'm.st'
  model_path.write_bytes(b'')
  model: db.AIModelType = _MakeModel(h='h1', name='m1', path=str(model_path))
...
def testReproduceModelNotInDBRaises(tmp_path: pathlib.Path) -> None:
  ...
  ai_db._db['images']['img-hash'] = entry
  api = _MockAPI()
  with pytest.raises(db.Error, match=r'missing-model.*not found in DB'):
    ai_db.Reproduce('img-hash', api)
